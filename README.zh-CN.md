...
│   │   └── xhs/             # 小红书用户/搜索/帖子详情采集 showcase
│   ├── app/
│   │   ├── integration/     # MCP / CLI / API 直连（不用屏幕或视觉识别）
│   │   └── desktop/
│   │       └── template_click/ # 无直连能力时的图像模板匹配点击
│   ├── mobile/
│   │   ├── android/
│   │   │   ├── adb_basics/  # Android ADB 基础：设备列表、截图、点击、滑动、推送文件
│   │   │   └── xhs_note/    # 小红书笔记草稿流程：ADB 慢节奏操作，默认停在最终发布前
│   │   └── ios/
│   │       └── iphone_assist/ # iPhone 准备与留证，最终 UI 操作保持人工确认
...
# 可选：先让 Agent 读取本机环境、可用 Skill 和安全边界
...
# 可选：浏览器需要登录/MFA 时，返回结构化人工交接而非在终端等待输入
python run.py harness/agent -- --goal "打开我的 CRM" --handoff-on-login
...
| `skill_solidify` | 将 trace JSON 固化为可监督首跑的 Skill，并返回语法检查和风险 review 清单 |
| `run_list` | 查询最近 Skill 运行记录或单次运行详情 |
| `runtime_snapshot` | 只读输出本机 Harness 环境、可用 Skill/能力、安全边界和下一步建议 |
...
| `showcase/web/xhs/user_posts` | Browser DOM 采集 | ✅ 可运行 | 慢滚动采集用户主页帖子；`--user-url <URL> --output data/xhs_user.json` |
| `showcase/web/xhs/search_posts` | Browser DOM 采集 | ✅ 可运行 | 慢滚动采集搜索词/tag；`--keyword "露营" --output data/xhs_search.json` |
| `showcase/web/xhs/post_detail` | Browser DOM 采集 | ✅ 可运行 | 采集单篇帖子可见文字、图片、视频、互动信息；`--url <URL>` |
| `showcase/app/desktop/template_click` | Desktop 图像模板匹配 | ✅ 可运行 | 没有 MCP/CLI/API 直连能力时的兜底；零 AI 成本；`--template assets/<系统>/<按钮>.png` |
| `showcase/mobile/android/adb_basics` | Android ADB 操作 | ✅ 可运行 | 需要 Android platform-tools / ADB；`--devices`、`--diagnostics`、`--tap-ratio 0.5 0.5` |
| `showcase/mobile/android/xhs_note` | Android ADB App 流程 | ✅ 可运行 | 慢节奏起草小红书笔记；默认停在最终发布前；`--profile data/xhs_profile.json --dry-run` |
| `showcase/mobile/ios/iphone_assist` | iPhone 半自动 | ✅ 可运行 | 可选依赖 `pymobiledevice3`；复制文案、打开 App、截图留证，最终操作人工确认 |
